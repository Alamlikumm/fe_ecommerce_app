...
import { ShoppingCart, Star, Eye, PackageX, Heart } from "lucide-react";
...
export default function ProductCard({ product, index = 0 }: ProductCardProps) {
    ...
    const handleToggleWishlist = async (e: React.MouseEvent) => {
        ...
        if (!token) {
            addToast({ type: "warning", title: "Login diperlukan", message: "Login untuk menyimpan wishlist." });
            toggleWishlist(product.id); // Rollback
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                ...
                addToast({
                    ...
                    title: data.wishlisted ? "Ditambahkan ke Wishlist ❤️" : "Dihapus dari Wishlist",
                    message: product.name,
                });
            } else {
                toggleWishlist(product.id); // Rollback on error
            }
        } catch {
            toggleWishlist(product.id); // Rollback on error
        }
                ...
                <button
                    ...
                    className={`absolute top-3 right-3 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 shadow-sm ${
                        wishlisted
                            ...
                            : "bg-white/90 dark:bg-gray-900/90 backdrop-blur-md text-gray-400 hover:text-red-500 border border-white/50 dark:border-gray-700"
                    }`}
                >
